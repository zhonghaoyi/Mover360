...
class FrechetAutoEncoderDistance(Metric):
    ...
    def __init__(self, pano_height: int):
        super().__init__()
        ckpt_path = os.path.join('checkpoints', 'faed.ckpt')
        # Always deserialize on CPU first so DDP test workers do not fight over
        # whatever device was recorded inside the checkpoint.
        faed = FAED.load_from_checkpoint(ckpt_path, map_location='cpu')
        self.encoder = faed.net.encoder.eval()
