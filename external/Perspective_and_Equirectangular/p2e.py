...
def p2e(p_img, fov_deg, u_deg, v_deg, out_hw, mode=None,Joinpoint=False):
    ...
    if isinstance(p_img, Tensor):
        ...
        equi = equi * mask
        if Joinpoint:
            equi = rearrange(equi, '(b m) c h w -> b m c h w', b=b)
            equi = equi.mean(dim=0)  # Stack all viewpoints
